...
int one_bit_branch_predictor(struct instruction branch, int last_prediction)
{
...
int branch_prediction_correctness(struct instruction IF, struct instruction ID)
{
    ...
    //the struct though
    //
    //if ID is not a branch, there is no control hazard
    if (ID.type != ti_BRANCH)
    {
        return 0;
    }
    //if IF pc address = id target address, the branch was taken, and our prediction was wrong
    if(IF.PC != ID.Addr)
    {
        return 1;
    }
...
int catch_data_hazard(struct instruction IF, struct instruction next) //returns 1 if there is a data hazard found + the ID instruction needs to be stalled.
{
    ...
    case ti_RTYPE: /* registers are translated for printing by subtracting offset  */
        if (next.sReg_a == IF.dReg | next.sReg_b == IF.dReg)
        {
            return 1;
        }
        break;
    case ti_ITYPE:
        if (next.sReg_a == IF.dReg)
        {
            return 1;
        }
        break;
    case ti_STORE:
        ...
        break;
    case ti_BRANCH:
        if (IF.dReg == next.sReg_a | IF.dReg == next.sReg_b)
        {
            return 1;
        }
        break;
    case ti_JRTYPE:
        if (IF.dReg == next.sReg_a)
        {
            return 1;
        }
        break;
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        ...
        if (!size)
        {
            memcpy(tr_entry, &NO_OP, sizeof(*tr_entry));
        }
        ...
        else               /* move the pipeline forward */
        {
            ...
            //always move MEM, EX, ID stage forward
            WB = MEM;
            MEM = EX;
            EX = ID;
            //check if prefetch queue has something in it
            //if it does, move it into IF, advance to next cycle
            if (pq.instr1.PC != 0)
            {
                ID = IF;
                IF = pq.instr1;
            ...
            else
            {
                ...
                if (prediction_method == 0)
                {
                    ...
                    control_hazard = branch_prediction_correctness(IF, ID);
                    if (control_hazard == 1)
                    {
                        //if there was a control hazard, move IF into prefetch queue and place a 'squashed' (NOP) instruction into IF
                        pq.instr1 = IF;
                        IF = get_NOP();
                    }
                }
                ...
                {
                    //use branch predictor hash table
                    prediction = one_bit_branch_predictor(ID, prediction);
                    //if 1, we predict the branch will be taken
                    //if 0, we predict the branch is not taken
                    //
                    //we need to check if our prediction was correct
                    control_hazard = branch_prediction_correctness(IF, ID);
                    if(control_hazard == 1)
                    {
                        //we made the wrong prediction
                        //move IF into prefetch queue
                        pq.instr1 = IF;
                        IF = get_NOP();
                    }
                ...
                if(control_hazard != 1)
                {
                    //found no control hazards, now check for data hazards
